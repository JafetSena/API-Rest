...
app.post("/alumnos", async (req, res, next) => {
    try{
        ...
        //res.send(req.body);
        let sent = `Insert Estudiante(nombre, apellido_paterno, apellido_materno, semestre) Values('${req.body.nombre}', '${req.body.apellido_paterno}', '${req.body.apellido_materno}', ${req.body.semestre} )`;
        ...
        let e = new Error("Error en la conexión de la base de datos");
        console.log(e);
        next(e);
    }
});

app.get("/alumnos", async (req, res, next) => {           //CONSULTA GENERAL
    try {
        const conn = await mysql.createConnection({ host: 'localhost', user: 'root', password: 'root', database: "test" });
